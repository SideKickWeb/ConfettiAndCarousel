...
// Optimize data fetching with lazy loading and caching
const productResponsePromise = useLazyFetch("/api/products", {
  server: false, // Load client-side only
  key: "products",
});
const categoryResponsePromise = useLazyFetch("/api/products/categories", {
  server: false, // Load client-side only
  key: "categories",
});
const instagramResponsePromise = useLazyFetch("/api/instagram", {
  server: false, // Load client-side only
  key: "instagram",
});

// Use a separate fetch for required hero component data only
// This will load on the server
...
const productResponse = computed(
  () => productResponsePromise.data.value || { data: [] }
);
const categoryResponse = computed(
  () => categoryResponsePromise.data.value || { data: [] }
);
const instagramResponse = computed(
  () => instagramResponsePromise.data.value || { data: [] }
);
  ...
  if (categories.value.length > 0) {
    categories.value.forEach((category) => {
      grouped[category.value] = {
        ...
        description:
          category.description || `Browse our ${category.label} collection`,
