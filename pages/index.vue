  ...
  <div class="page-wrapper">
    ...
    <main class="main-content">
      <section class="intro-section animate-section" ref="introSection">
        ...
        <p>Transform your special moments into unforgettable memories with our premium event decoration and hosting
          services. We specialize in creating bespoke experiences that reflect your unique style and vision.</p>
      </section>
      ...
      <section class="features animate-section" ref="featuresSection">
        <div class="feature-card">
          ...
          <p>Exquisite themes, premium balloon arrangements, elegant table settings, and stunning backdrops for
            sophisticated occasions.</p>
        </div>
        <div class="feature-card">
          <div class="icon">✧</div>
          <h3>Event Planning</h3>
          <p>Comprehensive event coordination from concept to execution, ensuring every detail meets our luxury
            standards.</p>
        </div>
        <div class="feature-card">
          <div class="icon">✧</div>
      ...
      <section class="services-highlight animate-section" ref="productsSection">
        ...
        <div class="carousel-container">
          <button class="carousel-button prev" @click="previousProduct" :disabled="!products || products.length <= 1">
            <span>❮</span>
          </button>
          
          <div class="service-carousel" ref="carousel">
            <div v-if="!productResponse || productResponse.loading" class="service-item active">
              <div class="loading-indicator">
                <span>Loading products...</span>
              </div>
            </div>
            <div v-else-if="!products || products.length === 0" class="service-item active">
              <h4>Products Coming Soon</h4>
              <p class="service-description">Our product catalog is being updated. Please check back soon!</p>
            </div>
            <template v-else>
              <div class="service-item" v-for="(category, index) in products" :key="category.id"
                  :class="{ 'active': currentProductIndex === index }">
                <h4>{{ category.title }}</h4>
                <p class="service-description">{{ category.description }}</p>
                <div class="service-items">
                  <div v-for="(item, itemIndex) in category.items?.slice(0, 3) || []" :key="item.id"
                    class="service-price-item">
                    <div class="item-header">
                      <span class="item-name">
                        <NuxtLink :to="`/product/${item.id}`" class="product-link">{{ item.name }}</NuxtLink>
                      </span>
                      <span class="item-price">£{{ typeof item.price === 'number' ? item.price.toFixed(2) : item.price
                        }}</span>
                    </div>
                    <p class="item-details">{{ item.description || 'Premium quality product' }}</p>
                  </div>
                  <p v-if="category.items && category.items.length > 3" class="more-items">+{{ category.items.length - 3
                    }} more items</p>
                </div>
                <NuxtLink :to="`/products#${category.anchor}`" class="service-link-button">View All {{ category.title }}
                </NuxtLink>
              </div>
            </template>
          </div>

          <button class="carousel-button next" @click="nextProduct" :disabled="!products || products.length <= 1">
            <span>❯</span>
          </button>
        </div>
        ...
        <div v-if="products && products.length > 1" class="carousel-indicators">
          <span v-for="(_, index) in products" :key="index" @click="currentProductIndex = index; centerActiveCard()"
            :class="{ active: currentProductIndex === index }" class="carousel-dot"></span>
        </div>
      </section>
      ...
      <section class="gallery-section">
        <div class="section-header">
          <h2>Our Gallery</h2>
          <p>Explore our collection of beautiful balloon arrangements and decorations</p>
        </div>

        <div class="gallery-stack">
          <button class="gallery-nav-button prev" @click="scrollGallery('left')" aria-label="Previous gallery card">
          ...
          <div class="gallery-card-wrapper" ref="galleryWrapper">
            <div v-for="(item, index) in galleryItems" :key="item.id || index" class="gallery-card" :class="{ 'active': selectedCard === index }">
              <a :href="item.link" target="_blank" rel="noopener noreferrer" v-if="item.link" @click.prevent="handleCardClick(index, item)">
                <img :src="item.image" :alt="item.title">
                <div class="gallery-card-content">
                  <div v-if="item.description" class="gallery-caption">{{ item.description }}</div>
                </div>
              </a>
              <div v-else class="gallery-card-container" @click="handleCardClick(index, item)">
                <img :src="item.image" :alt="item.title">
                <div class="gallery-card-content">
                  <div v-if="item.description" class="gallery-caption">{{ item.description }}</div>
                </div>
          ...
          <button class="gallery-nav-button next" @click="scrollGallery('right')" aria-label="Next gallery card">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <section class="booking-cta animate-section" ref="bookingSection">
        <div class="booking-content">
          ...
          <p>Book your consultation now and let our experts help you design the perfect experience for your special
            occasion.
          </p>
...
const { data: productResponse } = await useFetch('/api/products') || { data: ref({}) };
const { data: categoryResponse } = await useFetch('/api/products/categories') || { data: ref({}) };
  ...
  if (categories.value.length > 0) {
    categories.value.forEach(category => {
      grouped[category.value] = {
        ...
        description: category.description || `Browse our ${category.label} collection`,
        anchor: category.label.toLowerCase().replace(/\s+/g, '-'),
        items: []
      };
    });
  }
...
.gallery-card-wrapper::-webkit-scrollbar {
  display: none;
}
...
.gallery-card a, .gallery-card-container {
  position: relative;
  display: block;
  width: 100%;
  height: 100%;
...
@media (max-width: 768px) {
  ...
  .gallery-card-content {
    background: rgba(0, 0, 0, 0.85); /* Darker background for better readability on mobile */
  }
}
</style>
