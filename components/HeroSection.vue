...
// Accept hero data as a prop to avoid additional API calls
...
// Used only as fallback if heroData prop isn't provided
const fetchHeroData = async () => {
  try {
    return await useFetch("/api/hero", {
      key: "hero-component-data",
      cache: "force-cache",
    });
  } catch (error) {
    console.error("Error fetching hero data:", error);
    return {
      data: ref(props.heroData),
    };
  }
};

// Determine if we need to fetch or use the provided data
const { data: fetchedHeroData } = !props.heroData?.title
  ? await fetchHeroData()
  : { data: ref(props.heroData) };

// Use the data from prop or fallback to API fetch
const heroSettings = computed(() => {
  return props.heroData?.title
    ? props.heroData
    : fetchedHeroData.value || props.heroData;
});
